    ...
    try {
        ...
        if (catalogElement) {
            ...
            if (prices.length > 0) {
                ...
                priceRangeDiv.innerHTML = 'Fiyat bilgisi bulunamadı.';
            }
        } else {
...
async function fetchContentPriceBahceMarket(query) {
    ...
    try {
        ...
        const parser = new DOMParser();
        const doc = parser.parseFromString(text, 'text/html');
        const resultsDiv = document.getElementById('bahcem-market-result');
        const priceRangeDiv = document.getElementById('price-range');
        resultsDiv.innerHTML = '';
        priceRangeDiv.innerHTML = '';
    ...
    <div class="border rounded border-1 border-opacity-25 border-success mb-4 opacity-hover" style="width: 18rem; cursor: pointer;">
        ...
        <div class="card-body p-2">
            ...
            console.log('ProductPageProductList element not found');
            resultsDiv.textContent = 'Yok';
        }
    } catch (error) {
        console.error('Fetch error:', error);
        document.getElementById('bahcem-market-result').textContent = 'Bir hata oluştu.';
    }
}
